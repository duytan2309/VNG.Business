using System;

namespace VNG.Business.Entities.CuaHang
{
    public class CH_VanChuyen
    {
        public string TinhTrang { get; set; }
        public int Id { get; set; }
        public string MaNhapKho { get; set; }
        public DateTime? NgayXuat { get; set; }
        public string SoHoaDon { get; set; }
        public DateTime? NgayNhapKho { get; set; }
        public DateTime? NgayVanChuyen { get; set; }
        public string NoiXuat { get; set; }
        public string NoiNhap { get; set; }
        public string PhieuSx { get; set; }
        public decimal? GiaNhap { get; set; }
        public decimal? ThanhTien { get; set; }
        public string Kieu { get; set; }
        public string TenChungLoai { get; set; }
        public string Size { get; set; }
        public int SoLuong { get; set; }
        public string GhiChu { get; set; }
        public bool Isgot { get; set; }
        public DateTime? CreateDate { get; set; }
        public string CreateBy { get; set; }
        public int? TC { get; set; }
        public int? Size00 { get; set; }
        public int? Size34 { get; set; }
        public int? Size35 { get; set; }
        public int? Size36 { get; set; }
        public int? Size37 { get; set; }
        public int? Size38 { get; set; }
        public int? Size39 { get; set; }
        public int? Size40 { get; set; }
        public int? Size41 { get; set; }
        public int? Size42 { get; set; }
        public int? Size43 { get; set; }
        public int? Size44 { get; set; }
        public int? Size45 { get; set; }
        public int? Size46 { get; set; }
        public int? SLNhap { get; set; }
        public int? SLThieu { get; set; }
        public DateTime? NgayTuDongNhap { get; set; }
        public int? SoNgayVanChuyen { get; set; }
        public int? SoNgayVanChuyenQD { get; set; }
    }
}
